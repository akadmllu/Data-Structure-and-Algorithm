...
public class InsertOperation<T> {
	...
	private synchronized boolean insertNodeAtBegin(DoublyLinkedList<T> objList,Node<T> objNode) {
		objNode.setNext(objList.getHead());
		objList.getHead().setPrev(objNode);
		objList.objHead = objNode;
		objList.jLength++;
		return true;
	}
	//Insert the node at the end of list
	private synchronized boolean insertNodeAtEnd(DoublyLinkedList<T> objList,Node<T> objNode) {
	...
	private synchronized boolean insertNodeAtPosition(DoublyLinkedList<T> objList,Node<T> objNode,int jPosition) {
		if(jPosition < 0)
			jPosition = 0;
	...
	public void insertNode(DoublyLinkedList<T> objList)throws NumberFormatException,IOException {
