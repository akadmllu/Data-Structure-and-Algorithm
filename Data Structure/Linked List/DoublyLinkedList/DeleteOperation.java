...
public class DeleteOperation<T> {
	...
	private boolean deleteNodeAtBegin(DoublyLinkedList<T> objList) {
		if(objList.objHead == null){	
			System.out.println("List is empty");
			return false;
		}else {
			objList.objHead = objList.objHead.getNext();
			objList.objHead.setPrev(null);
			objList.jLength--;
			return true;			
		}
	}
	//Delete the node at the end of list
	private boolean deleteNodeAtEnd(DoublyLinkedList<T> objList) {
		...
		if(objList.objHead == null) {
			...
			objTempNode.setNext(null);
			objList.jLength--;
			return true;
		}
	}
	//Delete node at any position
	private boolean deleteNodeAtPosition(DoublyLinkedList<T> objList,int jPosition) {
		if(jPosition < 0)
			jPosition = 0 ;
		...
		if(objList.objHead == null)
		{
			...
			objTempNode.getNext().setPrev(objTempNode);
			objList.jLength--;
			return true;
		}
	}
	//delete node from list
	public void deleteNode(DoublyLinkedList<T> objList)throws NumberFormatException,IOException{
